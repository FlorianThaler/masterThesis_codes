...
def myDataFunc3d(x1, x2, x3):
	"""
		...
		@return ### retVal ### numpy array corresponding to the function values of the function to be
			approximated at (x1, x2, x3)
	
		this function generates function values of a known function, given points in the approximation domain
	"""
	
	# Friedman-type function ...
	# return 10 * np.sin(50 * x1 * x2) + 20 * (x3 - 0.25) ** 2
	
	# Rastrigin function
	return 30 + (x1 ** 2 - 10 * np.cos(2 * np.pi * x1)) + (x2 ** 2 - 10 * np.cos(2 * np.pi * x2)) + \
		(x3 ** 2 - 10 * np.cos(2 * np.pi * x3))
	
########################################################################################################################	

def my3dExample(L, rank, degrs, signature, subSignature, path2SubSignDir, path2FigDir,\
	...
	yData = myDataFunc3d(x1Data, x2Data, x3Data)
	...
	alsParamsDict['boundingBox_x3_right'] = boundingBox[2][0]
